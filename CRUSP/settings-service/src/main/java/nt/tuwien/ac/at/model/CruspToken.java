...
public class CruspToken {
    ...
    @NotNull
    @Max(10000) // 1.000 since backbone limitations at hossman.nt.tuwien.ac.at, 10.000 for 5G peak
    ...
    public static CruspToken from(CruspSettings cruspSettings) {
        CruspTokenBuilder builder = new CruspTokenBuilder();
        return builder.packetSize(cruspSettings.getPacketSize())
                .rate(cruspSettings.getRate())
                .repeats(cruspSettings.getRepeats())
                .sleep(cruspSettings.getSleep())
                .timeout(cruspSettings.getTimeout())
                .volume(cruspSettings.getVolume())
                .build();
    }
}
